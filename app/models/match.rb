class Match < ApplicationRecord
  ...
  after_update_commit :schedule_match_rating_jobs, if: :saved_change_to_kickoff_time?
  ...
  validates :team_score, numericality: { only_integer: true, greater_than_or_equal_to: 0 }, allow_nil: true

validates :opponent_score, numericality: { only_integer: true, greater_than_or_equal_to: 0 }, allow_nil: true
  ...
  def selected_player_ids
    squad_selections.pluck(:user_id)
  end

  def selected_players
    User.where(id: selected_player_ids)
  end
  ...
  def rating_target_ids_for(rater_id)
    player_ids = selected_player_ids

    if player_ids.include?(rater_id)
      player_ids - [rater_id]
    elsif approved_manager_ids.include?(rater_id)
      player_ids
    else
      []
    end
  ...
  def expected_rating_count_for(rater_id)
    rating_target_ids_for(rater_id).count
  end

  def ratings_submitted_by?(rater_id)
    expected_ids =
      rating_target_ids_for(rater_id)
    ...
    submitted_ids.sort == expected_ids.sort
  end

  def submitted_rating_rater_ids
    rating_rater_ids.select do |rater_id|
      ratings_submitted_by?(rater_id)
    end
  end
  ...
  def ratings_open_at
    kickoff_time + 2.hours
  end

  def ratings_close_at
    kickoff_time + 4.hours
  end
  ...
  def rating_results
    return [] unless ratings_finalised_at.present?
  ...
  def finalise_ratings!
    return if ratings_finalised_at.present?
  ...
  def schedule_match_rating_jobs
    ...
    MatchRatingOpenJob
      .set(wait_until: ratings_open_at)
      .perform_later(
        id,
        expected_kickoff_time
      )
    ...
    MatchRatingReminderJob
      .set(wait_until: ratings_open_at + 30.minutes)
      .perform_later(
        id,
        expected_kickoff_time
      )
    ...
    MatchRatingReminderJob
      .set(wait_until: ratings_open_at + 60.minutes)
      .perform_later(
        id,
        expected_kickoff_time
      )
    ...
    MatchRatingReminderJob
      .set(wait_until: ratings_open_at + 90.minutes)
      .perform_later(
        id,
        expected_kickoff_time
      )
    ...
    MatchRatingFinaliseJob
      .set(wait_until: ratings_close_at)
      .perform_later(
        id,
        expected_kickoff_time
      )
