...
class AppleAppStoreServerApiClient
  ...
  def validate_configuration!
    if issuer_id.blank?
      raise ConfigurationError,
            "Apple issuer ID is not configured"
    end

    if key_id.blank?
      raise ConfigurationError,
            "Apple key ID is not configured"
    end
  ...
  def authorization_token
    ...
    raise ConfigurationError,
          "Apple API authorization token could not be created: #{error.message}"
  end
  ...
  def signing_key!
    ...
    raise ConfigurationError,
          "Apple API private key is invalid: #{error.message}"
  end
  ...
  def parse_json(body)
    ...
    raise RequestFailed,
          "Apple App Store Server API returned invalid JSON: #{error.message}"
  end
end
